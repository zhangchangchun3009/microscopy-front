...
class OtaApkService {
  ...
  Future<void> startDownload({
    ...
    if (Platform.isAndroid) {
      try {
        final canInstall = await _channel.invokeMethod<bool>('checkInstallPermission') ?? false;
        if (!canInstall) {
    ...
    try {
      ...
      dlStopwatch.stop();
      debugPrint('[OTA] 下载完成，耗时 ${dlStopwatch.elapsedMilliseconds}ms，文件大小: ${file.lengthSync()} bytes');
